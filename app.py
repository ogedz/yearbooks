...
# ---------------------------------------------------------------------------
# Subpath support (e.g. running behind lifox.com.ng/yearbook via a
# Cloudflare Worker reverse proxy). Set the URL_PREFIX env var on Render to
# "/yearbook" and this makes url_for(), redirects, and static asset paths
# all resolve correctly under that prefix. Leave URL_PREFIX unset to run
# at the domain root as before (e.g. local dev, or a direct *.onrender.com URL).
# ---------------------------------------------------------------------------
URL_PREFIX = os.environ.get('URL_PREFIX', '').rstrip('/')

class PrefixMiddleware:
    """Wraps the WSGI app so it correctly handles being mounted under a path
    prefix forwarded by an upstream proxy (Cloudflare Worker, Nginx, etc.)."""
    ...
    def __call__(self, environ, start_response):
        if self.prefix:
            ...
            elif path == self.prefix.rstrip('/') or path == '':
                environ['SCRIPT_NAME'] = self.prefix
                environ['PATH_INFO'] = '/'
                return self.wsgi_app(environ, start_response)
        return self.wsgi_app(environ, start_response)

# Trust the X-Forwarded-* headers Cloudflare/the proxy sets, so request.url,
# request.scheme etc. are correct (https, real host) instead of reflecting
# the internal Render container's view of the request.
